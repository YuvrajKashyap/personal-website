...
const askAiHref = `https://chatgpt.com/?${new URLSearchParams({
  hints: "search",
  q: chatGptPrompt,
  "temporary-chat": "true",
}).toString()}`;
...
const heroSocialOrder = ["GitHub", "LinkedIn", "X", "YouTube", "Instagram"];

function getHeroActions(): HeroAction[] {
  const socialActions = heroSocialOrder.flatMap((label) => {
...
export function HeroActionLinks() {
  return (
    <div className="hero-action-list" aria-label="Primary external links">
      {getHeroActions().map((action) => (
        <a
          key={action.id}
          href={action.href}
          aria-label={action.ariaLabel}
          className="hero-action-button focus-ring"
